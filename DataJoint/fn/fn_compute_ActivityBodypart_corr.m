function fn_compute_ActivityBodypart_corr(key,self, rel_data,threshold_for_event)
smooth_window=3;
...
[bodypart_vector,idx_frames] = fn_compute_corr_with_bodypart (key, frame_rate, smooth_window);
...
for i_thr=1:1:numel(threshold_for_event)
    ...
    for i_roi=1:1:size(S,1)
        ...
        key_ROI1(i_roi).bodypart_name = key.bodypart_name;
        
        %% correlation with bodypart
        trace=S(i_roi).dff_trace;
        trace(trace<threshold_for_event_temp(i_thr)) =0;
        trace = smooth(trace,smooth_window);
        r = corr([trace(idx_frames),bodypart_vector'],'rows','pairwise');
        key_ROI1(i_roi).corr_with_bodypart = r(2);
