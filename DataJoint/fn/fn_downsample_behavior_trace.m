function [body_vector,idx_2pimaging_frames]= fn_downsample_behavior_trace(key,rate_behavior, rate_2p, start_frame_2p, predictor_name)
smooth_body_window=3;
key.bodypart_name=predictor_name;
switch key.bodypart_name
    case 'pawfrontleft' %front view
        VIDEO= fetchn(rel_video & key,'traj_x','ORDER BY trial');%
        k.tracking_device_id=4;
    case 'pawfrontright' %front view
        VIDEO= fetchn(rel_video& key,'traj_x','ORDER BY trial');%
        k.tracking_device_id=4;
    case 'whiskers' %front view
        VIDEO= fetchn(rel_video& key,'traj_y1','ORDER BY trial');%
        k.tracking_device_id=4;
end
trial_num_video = fetchn(rel_video & key,'trial','ORDER BY trial');
t_first_video_frame = fetchn(rel_video& key,'time_first_frame','ORDER BY trial') + fetchn(rel_video & key,'lickport_t_entrance_relative_to_trial_start','ORDER BY trial'); % relative to trial start
tracking_sampling_rate_trials =fetchn(rel_video*TRACKING.TrackingTrial & k & key,'tracking_sampling_rate','ORDER BY trial');
...
for i_tr = 1:1:numel(trial_num)-1
    ...
    current_body_vector_smoothed =smooth(current_body_vector,smooth_body_window); %to interploate and eliminate NaNs
    ...
    for ii=1:1:numel(time_2pimaging)-1
        ...
        current_body_vector_resampled(ii) =  nanmedian(current_body_vector_smoothed(idx_b)); %downsampling, to match the imaging frame rate
    end
    body_vector= [body_vector; current_body_vector_resampled'];
    
    clf
    hold on
    plot(current_body_vector_smoothed,'-b')
    plot(current_body_vector,'-r')
