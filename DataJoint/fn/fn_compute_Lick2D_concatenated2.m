function fn_compute_Lick2D_concatenated2(key,self, rel_data, fr_interval)
...
parfor i_roi=1:1:size(S,1)
    ...
    for i_x=1:1:numel(x_bins_centers)
        for i_z=1:1:numel(z_bins_centers)
            ...
            if numel(idx)<timespent_min
                psth_position_concat(:,pos_counter) =  (time+NaN);
                psth_position_concat_odd (:,pos_counter)= [(time+NaN)];
                psth_position_concat_even (:,pos_counter)= [(time+NaN)];
                psth_position_concat_regularreward (:,pos_counter)= [(time+NaN)];
                psth_position_concat_regularreward_odd (:,pos_counter)=  [(time+NaN)];
                psth_position_concat_regularreward_even (:,pos_counter)= [(time+NaN)];
            else
                psth_position_concat (:,pos_counter)= [double(mean(cell2mat([psth_all(idx)]'),1))];
                psth_position_concat_odd (:,pos_counter)= [double(mean(cell2mat([psth_all(ismember([1:1:num_trials],idx) & idx_odd)]'),1))];
                psth_position_concat_even (:,pos_counter)= [double(mean(cell2mat([psth_all(ismember([1:1:num_trials],idx) & idx_even)]'),1))];
                
                try
                    psth_position_concat_regularreward (:,pos_counter)= [double(mean(cell2mat([psth_all(idx)& idx_regular]'),1))];
                    psth_position_concat_regularreward_odd (:,pos_counter)= [double(mean(cell2mat([psth_all(ismember([1:1:num_trials],idx) & idx_odd & idx_regular)]'),1))];
                    psth_position_concat_regularreward_even (:,pos_counter)= [double(mean(cell2mat([psth_all(ismember([1:1:num_trials],idx) & idx_even & idx_regular)]'),1))];
                catch
                    psth_position_concat_regularreward (:,pos_counter)= [double(mean(cell2mat([psth_all(idx)]'),1))];
                    psth_position_concat_regularreward_odd (:,pos_counter)= [double(mean(cell2mat([psth_all(ismember([1:1:num_trials],idx) & idx_odd)]'),1))];
                    psth_position_concat_regularreward_even (:,pos_counter)= [double(mean(cell2mat([psth_all(ismember([1:1:num_trials],idx) & idx_even)]'),1))];
                end
            end
    ...
    % Computer correlation across all target position bins
    ...
    % Here we also compute correlation separately for each target position bin and then averaged across bins. This is too counteract effect of direction and camture only temporal tuning
    
    for i_b=1:size(psth_position_concat_odd,2)
        r_temp=corr([psth_position_concat_odd(:,i_b),psth_position_concat_even(:,i_b)],'Rows' ,'pairwise');
        psth_position_concat_odd_even_corr_binwise(i_b) =r_temp(2);
    end
    psth_position_concat_odd_even_corr_binwise = nanmean(psth_position_concat_odd_even_corr_binwise);

    
    for i_b=1:size(psth_position_concat_regularreward_odd,2)
        r_temp=corr([psth_position_concat_regularreward_odd(:,i_b),psth_position_concat_regularreward_even(:,i_b)],'Rows' ,'pairwise');
        psth_position_concat_regularreward_odd_even_corr_binwise(i_b) =r_temp(2);
    end
    psth_position_concat_regularreward_odd_even_corr_binwise = nanmean(psth_position_concat_regularreward_odd_even_corr_binwise);
    ...
    key_ROI1(i_roi).psth_position_concat_odd_even_corr_binwise = psth_position_concat_odd_even_corr_binwise;
    ...
    key_ROI1(i_roi).psth_position_concat_regularreward_odd_even_corr_binwise = psth_position_concat_regularreward_odd_even_corr_binwise;
