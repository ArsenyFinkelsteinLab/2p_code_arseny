function [map_f_smoothed, map_dff_smoothed] = fn_compute_map_dff (map,  smoothing_2D_size, baselineF_2D, z_score_threshold)
...
for i_fr=1:1:size(map,3)
    ...
    dff=(f-baselineF_2D)./baselineF_2D;
    dff(idx_remove)=0; % remove small values (noise)
    
    idx_outliers = abs(zscore(dff(:)))>3;
...
map_f_smoothed = smooth3(map,'gaussian',smoothing_3D_size); %3D smoothing in x-y-z
