function PLOTS_Maps2DBlock(key, dir_current_fig,flag_spikes, flag_block)
close all;
% clf;
...
%     rel_stats=LICK2D.ROILick2DPSTHStatsSpikes*LICK2D.ROILick2DangleSpikes;
    ...
    rel_stats=LICK2D.ROILick2DBlockStats*LICK2D.ROILick2DPSTHStats*LICK2D.ROILick2Dangle;
end


switch flag_block
    case 0 %first versus begin
        rel_rois=  IMG.ROIGood & (rel_stats & key  & 'block_mean_pval_first_begin<0.05');
        filename_suffix = 'FirstVsBegin';
    case 1 %regular first versus end
        rel_rois=  IMG.ROIGood & (rel_stats & key  & 'block_mean_pval_first_end<0.05');
        filename_suffix = 'FirstVsEnd';
    case 2 %regular begin versus end
        rel_rois=  IMG.ROIGood & (rel_stats & key  & 'block_mean_pval_begin_end<0.05');
        filename_suffix = 'BeginVsEnd';
end
...
switch flag_block
    case 0 %first versus begin
        x=M.block_mean_begin;
        y=M.block_mean_first;
    case 1 %regular first versus end
        x=M.block_mean_end;
        y=M.block_mean_first;
    case 2 %regular begin versus end
        x=M.block_mean_end;
        y=M.block_mean_begin;
end
...
%% Map of In-block order response
...
%% In block order preference histogram
...
% ylim([0 ceil(max(y))]);
ylabel(sprintf('Order-modulated\n neurons (%%)'),'Color',[0.5 0.5 0.5]);
xlabel(sprintf('In block order modulation, primacy (%% increase)'));
...
%% Preferred time histogram, for block-order increasing or decreasing cells
axes('position',[position_x2(4)+0.15, position_y2(2), panel_width2, panel_height2]);
...
ylim([0 ceil(max(y))]);
ylabel(sprintf('Primacy-enhanced\n neurons (%%)'),'Color',[1 0 0]);
...
ylabel(sprintf('Primacy-suppressed\n neurons (%%)'),'Color',[0 0 1]);
...
switch flag_block
    case 0 %first versus begin
PSTH2 = cell2mat(fetchn(rel_psth, 'psth_begin', 'ORDER BY roi_number'));
PSTH3 = cell2mat(fetchn(rel_psth, 'psth_first', 'ORDER BY roi_number'));
    case 1 %regular first versus end
PSTH2 = cell2mat(fetchn(rel_psth, 'psth_end', 'ORDER BY roi_number'));
PSTH3 = cell2mat(fetchn(rel_psth, 'psth_first', 'ORDER BY roi_number'));
    case 2 %regular begin versus end
PSTH2 = cell2mat(fetchn(rel_psth, 'psth_end', 'ORDER BY roi_number'));
PSTH3 = cell2mat(fetchn(rel_psth, 'psth_begin', 'ORDER BY roi_number'));
end
