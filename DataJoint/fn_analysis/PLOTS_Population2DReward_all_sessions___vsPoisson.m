function PLOTS_Population2DReward_all_sessions___vsPoisson
close all;
...
filename=['all_sessions_real_vs_poisson'];
...
for i=1:1:length(PSTH)
    max_PSTH= max(PSTH(i).psth);
    [~,idx_max_large] = max(PSTH(i).psth_large);
    large_peak_real(i) = PSTH(i).psth_large(idx_max_large)./max_PSTH;
    large_peak_posson(i) = PSTH(i).psth_large_poisson(idx_max_large)./max_PSTH;
end
...
for i=1:1:length(PSTH)
    max_PSTH= max(PSTH(i).psth);
    [~,idx_max_small] = max(PSTH(i).psth_small);
    small_peak_real(i) = PSTH(i).psth_small(idx_max_small)./max_PSTH;
    small_peak_posson(i) = PSTH(i).psth_small_poisson(idx_max_small)./max_PSTH;
end
