function PLOTS_Population2D_preferred_position
close all;

number_of_bins=4;
key = (EXP2.Session & 'session>=1') &  LICK2D.ROILick2DangleSpikes & (LICK2D.ROILick2DmapSpikes & sprintf('number_of_bins=%d',number_of_bins)) - IMG.Mesoscope;

rel_all=(LICK2D.ROILick2DmapSpikes*LICK2D.ROILick2DangleSpikes & key) & 'theta_tuning_odd_even_corr>=0.5' & 'goodness_of_fit_vmises>=0.5';



dir_base = fetch1(IMG.Parameters & 'parameter_name="dir_root_save"', 'parameter_value');
dir_current_fig = [dir_base  '\Lick2D\population\preferred_position_not_meso\'];
...
preferred_theta_vmises=fetchn(rel_all ,'preferred_theta_vmises');

subplot(2,2,1)
histogram(preferred_theta_vmises)
