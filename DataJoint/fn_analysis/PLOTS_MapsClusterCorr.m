function PLOTS_MapsClusterCorr(key, flag_plot_clusters_individually, n_clust, M_all)
close all;
...
filename=[];
dir_current_fig = [dir_base  '\Lick2D\brain_maps\clusters_corr\'];
rel = IMG.ROI &  ((POP.ROIClusterCorr& key) & 'heirar_cluster_percent>=1')  & IMG.ROIGood ;


rel1 = ((IMG.ROI*POP.ROIClusterCorr*IMG.PlaneCoordinates) & IMG.ROIGood & key) & 'heirar_cluster_percent>=1';
...
for i_clus = 1:1:min([numel(unique_cluster_id),10])
    ...
    title(sprintf('%s %d\n anm%d session %d,  Total planes = %d\nsplits = %d  %.1f%% cells shown', session_epoch_label, key.session_epoch_number, key.subject_id,key.session, number_of_planes, n_clust, percent_clustered));
