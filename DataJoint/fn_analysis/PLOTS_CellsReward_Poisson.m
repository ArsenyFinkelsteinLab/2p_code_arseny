function PLOTS_CellsReward_Poisson(key, dir_current_fig, flag_spikes, plot_one_in_x_cell)
close all;
...
if flag_spikes==0 % if based on dff
 rel_rois=  (IMG.ROI - IMG.ROIBad) & (LICK2D.ROILick2DRewardStatsSpikes  & key    & 'reward_mean_pval_regular_small<=0.01 OR reward_mean_pval_regular_large<=0.01 ');
    rel_map_and_psth = LICK2D.ROILick2DmapSpikes * LICK2D.ROILick2DSelectivitySpikes * LICK2D.ROILick2DPSTHSpikesPoisson * LICK2D.ROILick2DQuadrants  & rel_rois;
    ...
    %     rel_map_and_psth = LICK2D.ROILick2DmapSpikes * LICK2D.ROILick2DSelectivitySpikes * LICK2D.ROILick2DPSTHSpikesPoisson  * rel_rois;
    %     rel_angle = LICK2D.ROILick2DangleSpikes  * rel_rois;
    %     rel_stats = LICK2D.ROILick2DPSTHStatsSpikes*LICK2D.ROILick2DSelectivityStatsSpikes  * rel_rois;
    rel_rois=  (IMG.ROI - IMG.ROIBad) & (LICK2D.ROILick2DRewardStatsSpikes  & key   & 'reward_mean_pval_regular_small<=0.01 OR reward_mean_pval_regular_large<=0.01 ');
    rel_map_and_psth = LICK2D.ROILick2DmapSpikes * LICK2D.ROILick2DSelectivitySpikes * LICK2D.ROILick2DPSTHSpikesPoisson * LICK2D.ROILick2DQuadrantsSpikes  & rel_rois;
    rel_angle = LICK2D.ROILick2DangleSpikes  & rel_rois;
    rel_stats = LICK2D.ROILick2DPSTHStatsSpikes *LICK2D.ROILick2DSelectivityStatsSpikes*LICK2D.ROILick2DRewardStatsSpikes  & rel_rois;
...
M.psth_small_poisson=[fetchn(rel_map_and_psth,'psth_small_poisson','ORDER BY roi_number')];
M.psth_regular_poisson=[fetchn(rel_map_and_psth,'psth_regular_poisson','ORDER BY roi_number')];
M.psth_large_poisson=[fetchn(rel_map_and_psth,'psth_large_poisson','ORDER BY roi_number')];
...
%     psth_max= psth_max(:,psthmap_time>-2 & psthmap_time<=3);
    ...
    xlim(xl);
    ylim([0, 1]);
    title(sprintf('                 Omission p = %.3f Increase p = %.3f over all bins \n              Omission p = %.3f Increase p = %.3f at peak', M.reward_mean_pval_regular_small(i_roi),M.reward_mean_pval_regular_large(i_roi), M.reward_peak_pval_regular_small(i_roi),M.reward_peak_pval_regular_large(i_roi)), 'FontSize',10);
    xlabel('Time to lick (s)', 'FontSize',10);
    ylabel('Normalized response', 'FontSize',10);
    set(gca,'XTick',[xl(1),0,xl(end)],'Ytick',[0, 1],'TickLength',[0.05,0], 'FontSize',10);
...
%     text(5, 0.3, sprintf('Reward omission\n'),'Color',[0 0.7 0.2]);
    ...
    %% PSTH by Reward size Poisson
    axes('position',[position_x2(2),position_y2(2), panel_width2, panel_height2])
    hold on;
    psth_small_poisson= M.psth_small_poisson{i_roi};
    psth_regular_poisson= M.psth_regular_poisson{i_roi};
    psth_large_poisson= M.psth_large_poisson{i_roi};

    psth_small_poisson = smooth(psth_small_poisson/psth_reward_max,smooth_bin)';
    psth_regular_poisson = smooth(psth_regular_poisson/psth_reward_max,smooth_bin)';
    psth_large_poisson = smooth(psth_large_poisson/psth_reward_max,smooth_bin)';
    
    plot(psth_time,psth_regular_poisson,'-','Color',[ 0 0 0],'LineWidth',2);
    plot(psth_time,psth_large_poisson,'-','Color',[ 1 0.5 0],'LineWidth',2);
    plot(psth_time,psth_small_poisson,'-','Color',[0 0.7 0.2],'LineWidth',2);

    xl = [floor(psth_time(1)) ceil(psth_time(end))];
    xlim(xl);
    ylim([0, 1]);
    title(sprintf('Poisson'), 'FontSize',10);
    xlabel('Time to lick (s)', 'FontSize',10);
    ylabel('Normalized response', 'FontSize',10);
    set(gca,'XTick',[xl(1),0,xl(end)],'Ytick',[0, 1],'TickLength',[0.05,0], 'FontSize',10);
