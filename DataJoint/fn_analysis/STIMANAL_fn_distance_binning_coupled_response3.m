function key =STIMANAL_fn_distance_binning_coupled_response3(rel, key,DATA_all,rel_group_targets, p_val, distance_lateral_bins,  column_radius,minimal_distance, flag_withold_trials, flag_normalize_by_total,flag_divide_by_std, flag_prominent)
rel = rel & key;
...
DATA=fetch((rel & rel_group_targets) ,'*'); % we take all the cell pairs except those within 20 micron of stimulation laterally (regardless of depth)

F_mean_odd=[DATA.response_mean_odd]';
if flag_prominent==1
    factor=0.5;
    ix_prominent = (([DATA.response_max_odd] > factor*[DATA.non_response_max_odd])' &  F_mean_odd>0) | (([DATA.response_min_odd] < factor*[DATA.non_response_min_odd])' &  F_mean_odd<0);
    DATA=DATA(ix_prominent);
elseif flag_prominent==2
    factor=0.75;
    ix_prominent = (([DATA.response_max_odd] > factor*[DATA.non_response_max_odd])' &  F_mean_odd>0) | (([DATA.response_min_odd] < factor*[DATA.non_response_min_odd])' &  F_mean_odd<0);
    DATA=DATA(ix_prominent);
elseif flag_prominent==3
    factor=1;
    ix_prominent = (([DATA.response_max_odd] > factor*[DATA.non_response_max_odd])' &  F_mean_odd>0) | (([DATA.response_min_odd] < factor*[DATA.non_response_min_odd])' &  F_mean_odd<0);
    DATA=DATA(ix_prominent);
end
