function PLOTS_MapsClusterCorrSVD_CCF(key, flag_plot_clusters_individually,  M_all, dir_current_fig)
close all;
...
rel = rel_roi &  ((POP.ROIClusterCorrSVD& key) & 'heirar_cluster_percent>=1');
rel1 = ((rel_roi*POP.ROIClusterCorrSVD*IMG.PlaneCoordinates)  & key) & 'heirar_cluster_percent>=1';
...
my_colormap(end+1,:)=[1 0 0];
my_colormap(end+1,:)=[0 0 1];
my_colormap(end+1,:)=[0 0.8 0];
my_colormap(end+1,:)=[0.94 0.9 0];
my_colormap(end+1,:)=[0 0.8 0.8];
my_colormap(end+1,:)=[1 0 1];
my_colormap(end+1,:)=[0.9100    0.4100    0.1700];
my_colormap(end+1,:)=[0 1 1];
my_colormap(end+1,:)=[0.2 0.2 0.2];
...
if ~isempty(bregma_x_mm)
    ...
    x=x-[x_all_max - bregma_x_mm]; % anterior posterior
    y=y-y_all_min+bregma_y_mm; % medial lateral
...
for i_clus = 1:1:min([numel(unique_cluster_id),10])
    ...
    axis tight
    axis equal
    xlabel('Posterior - Anterior (\mum)');
    ylabel('Medial - Lateral (\mum)');
    percent_clustered = 100*numel(roi_cluster_id)/size(M_all,1);
    title(sprintf('%s %d\n anm%d session %d,  Total planes = %d\nsplits = %d, threshold =%.2f %.1f%% cells shown\n%d SVD components removed\n', session_epoch_label, key.session_epoch_number, key.subject_id,key.session, number_of_planes, n_clust, threshold_for_event, percent_clustered,svd_removed));
