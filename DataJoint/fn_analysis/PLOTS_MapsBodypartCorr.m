function PLOTS_MapsBodypartCorr(key, dir_current_fig,flag_spikes)
close all;
% clf;

if flag_spikes==1
else
        rel_data=LICK2D.ROIBodypartCorr;
end
...
R = fetchn(rel_data,'corr_with_bodypart','ORDER BY roi_number');
...
step =mean(diff(v));
hist_bins1 = [-1,-bounds:step:bounds,1];
...
title([ 'anm' num2str(key.subject_id) ' s' num2str(key.session) ' ' session_date ' Correlation with ' key.bodypart_name]);
% Colorbar
...
dir_current_fig = [dir_current_fig key.bodypart_name '\'];
