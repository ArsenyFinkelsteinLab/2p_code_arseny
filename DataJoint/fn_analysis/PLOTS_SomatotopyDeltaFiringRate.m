function PLOTS_SomatotopyDeltaFiringRate(key)
close all;

dir_base = fetch1(IMG.Parameters & 'parameter_name="dir_root_save"', 'parameter_value');

dir_current_fig = [dir_base  '\Lick2D\brain_maps\somatotopy\delta_firing_rate\'];
dir_current_fig  = [dir_current_fig sprintf('%.2f',key.threshold_for_event) '\'];

rel = IMG.ROI* IMG.ROIdeltaFPeak*IMG.PlaneCoordinates & key;

epoch_duration = fetchn(EXP2.SessionEpoch & key,'session_epoch_end_time') - fetchn(EXP2.SessionEpoch & key,'session_epoch_start_time');


key2 = rmfield(key,'session_epoch_number');
rel_control = IMG.ROI* IMG.ROIdeltaFPeak*IMG.PlaneCoordinates &  key2 & (EXP2.SessionEpochSomatotopy & 'sensory_stimulation_area="control"');
rel_control_duration = EXP2.SessionEpoch &  key2 & (EXP2.SessionEpochSomatotopy & 'sensory_stimulation_area="control"');
epoch_duration_control= fetchn(rel_control_duration,'session_epoch_end_time') - fetchn(rel_control_duration,'session_epoch_start_time');

if rel_control.count ==0
    rel_control = (IMG.ROI* IMG.ROIdeltaFPeak*IMG.PlaneCoordinates &  key2) - EXP2.SessionEpochSomatotopy;
    rel_control_duration = (EXP2.SessionEpoch &  key2)  - EXP2.SessionEpochSomatotopy;
    epoch_duration_control= fetchn(rel_control_duration,'session_epoch_end_time') - fetchn(rel_control_duration,'session_epoch_start_time');
end
...
x1 = M.number_of_events./epoch_duration;
x2 = M2.number_of_events./epoch_duration_control;
