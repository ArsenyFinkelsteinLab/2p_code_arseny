function PLOTS_MapsRidge(key, dir_current_fig, rel_data,tt, bounds, M, R, time_shift)
key.time_shift=time_shift;
...
v = linspace(-bounds,bounds,20);
step =mean(diff(v));
hist_bins1 = [-inf,-bounds:step:bounds,inf];

ax1=axes('position',[position_x1(1), position_y1(1), panel_width1, panel_height1]);
hold on;
my_colormap=redblue(numel(hist_bins1)-1);
my_colormap=flip(my_colormap);
...
title(sprintf('anm %d s%d %s \n %s       Regression beta \n t = %.2f s relative to event', key.subject_id, key.session, session_date, key.predictor_name, ((key.time_shift-1)/sampling_rate_2p)  ));
...
filename =[filename '_' sprintf('%s_threshold%d_t%d',key.predictor_name,floor(100*(key.threshold_for_flourescence_event)),tt)];
