...
classdef DistanceCorrPSTHSpikes < dj.Computed
    properties
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            psth_regular_odd_even_corr=[-1, 0, 0.25, 0.5];
            
            mesoscope_flag=count(IMG.Mesoscope & key);
                     if mesoscope_flag==1
                dir_save_fig = [dir_base  '\Lick2D\population\TUNING_vs_DISTANCE\single_sessions\tuning_by_psth\'];
            else
            ...
            for i_c = 1:1:numel(psth_regular_odd_even_corr)
                rel_roi = (IMG.ROI - IMG.ROIBad) & key & (LICK2D.ROILick2DPSTHStatsSpikes & sprintf('psth_regular_odd_vs_even_corr>=%.2f',psth_regular_odd_even_corr(i_c)));
