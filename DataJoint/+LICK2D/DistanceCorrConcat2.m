...
classdef DistanceCorrConcat2 < dj.Computed
    properties
                keySource = ((EXP2.SessionEpoch  & IMG.ROI & LICK2D.ROILick2DContactenated - IMG.Mesoscope) - EXP2.SessionEpochSomatotopy)&IMG.Volumetric ;
%         keySource = ((EXP2.SessionEpoch  & IMG.ROI & LICK2D.ROILick2DContactenated) - EXP2.SessionEpochSomatotopy)&IMG.Volumetric ;
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            if mesoscope_flag==1
                dir_save_fig = [dir_base  '\Lick2D\population\corr_distance_concat_meso_dff\'];
            else
                dir_save_fig = [dir_base  '\Lick2D\population\corr_distance_concat_notmeso_dff_ETL2\'];
                
            end
            
            for i_c = 1:1:numel(psth_position_concat_regularreward_odd_even_corr)
                rel_roi = (IMG.ROI - IMG.ROIBad) & key & (LICK2D.ROILick2DContactenated & sprintf('psth_position_concat_regularreward_odd_even_corr>=%.2f',psth_position_concat_regularreward_odd_even_corr(i_c)));
                rel_roi_xy = IMG.ROIPositionETL2 & rel_roi;
                rel_data = LICK2D.ROILick2DContactenated & rel_roi & key;
