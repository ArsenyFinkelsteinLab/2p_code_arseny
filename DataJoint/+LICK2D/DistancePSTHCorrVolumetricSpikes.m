...
classdef DistancePSTHCorrVolumetricSpikes < dj.Computed
    properties
        keySource = ((EXP2.SessionEpoch  & IMG.ROI & LICK2D.ROILick2DPSTHSpikes & IMG.Mesoscope) - EXP2.SessionEpochSomatotopy)&IMG.Volumetric ;
    end
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            dir_save_fig = [dir_base  '\Lick2D\population\corr_distance_psth\'];
            
            psth_odd_even_corr_threshold=[-1, 0, 0.25, 0.5, 0.75];
            
            for i_c = 1:1:numel(psth_odd_even_corr_threshold)
                rel_roi = (IMG.ROI - IMG.ROIBad) & key & (LICK2D.ROILick2DPSTHStatsSpikes & sprintf('psth_odd_even_corr>=%.2f',psth_odd_even_corr_threshold(i_c)));
                rel_data = LICK2D.ROILick2DPSTHSpikes & rel_roi & key;
                key.odd_even_corr_threshold = psth_odd_even_corr_threshold(i_c);
