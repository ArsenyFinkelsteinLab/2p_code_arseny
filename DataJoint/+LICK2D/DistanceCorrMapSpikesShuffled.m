...
classdef DistanceCorrMapSpikesShuffled < dj.Computed
    properties
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            mesoscope_flag=count(IMG.Mesoscope & key);
                if mesoscope_flag==1
                dir_save_fig = [dir_base  '\Lick2D\population\TUNING_vs_DISTANCE\single_sessions\tuning_by_map_fr\shuffled\'];
            else
                dir_save_fig = [dir_base  '\Lick2D\population\TUNING_vs_DISTANCE\single_sessions_mesoscope\tuning_by_map_fr\shuffled\'];
            end
