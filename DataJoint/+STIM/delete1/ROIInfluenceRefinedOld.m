...
classdef ROIInfluenceRefinedOld < dj.Computed
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            distance_to_exclude_other=100; %microns
            distance_to_exclude_control=25; %microns
...
%             rel_data = IMG.ROIdeltaF;
            ...
            group_list = fetchn((IMG.PhotostimGroup & key & STIMANAL.NeuronOrControl),'photostim_group_num','ORDER BY photostim_group_num');
...
%             F_binned = F_binned-mean(F_binned,2);
            ...
            rel_control_sites=IMG.PhotostimGroup & (STIMANAL.NeuronOrControl & key & 'neurons_or_control=0');
            rel_control_sites=rel_control_sites* IMG.PhotostimGroupROI;
            controlsites_photostim_frames =(fetchn(rel_control_sites,'photostim_start_frame','ORDER BY photostim_group_num')');
            controlsites_num =(fetchn(rel_control_sites,'photostim_group_num','ORDER BY photostim_group_num')');
            controlsites_center_x =(fetchn(rel_control_sites,'photostim_center_x','ORDER BY photostim_group_num')');
            controlsites_center_y =(fetchn(rel_control_sites,'photostim_center_y','ORDER BY photostim_group_num')');
            
            rel_other_sites=(IMG.PhotostimGroup& key) - (STIMANAL.NeuronOrControl & key & 'neurons_or_control=0');
            rel_other_sites=rel_other_sites* IMG.PhotostimGroupROI;

            othersites_photostim_frames =(fetchn(rel_other_sites,'photostim_start_frame','ORDER BY photostim_group_num')'); 
            othersites_num =(fetchn(rel_other_sites,'photostim_group_num','ORDER BY photostim_group_num')');
            othersites_center_x =(fetchn(rel_other_sites,'photostim_center_x','ORDER BY photostim_group_num')');
            othersites_center_y =(fetchn(rel_other_sites,'photostim_center_y','ORDER BY photostim_group_num')');
            ...
            for i_c = 1:1:numel(num_svd_components_removed_vector)
               ...
               parfor i_g = 1:1:numel(group_list)
                    ...
                    target_photostim_frames=ceil(target_photostim_frames./bin_size_in_frame);
                    ...
                    for i_r= 1:1:numel(roi_list)
                        ...
                        dx = controlsites_center_x - R_x(i_r);
                        dy = controlsites_center_y - R_y(i_r);
                        distance2D = sqrt(dx.^2 + dy.^2)*pix2dist; %
                        idx_controlsites_far = distance2D>distance_to_exclude_control   & ~ (controlsites_num== group_list(i_g));
                        controlsites_photostim_frames_far = cell2mat(controlsites_photostim_frames(idx_controlsites_far));
                        controlsites_photostim_frames_far = unique(ceil(controlsites_photostim_frames_far./bin_size_in_frame));
                       
                        idx_controlsites_near = distance2D<=distance_to_exclude_control   & ~ (controlsites_num== group_list(i_g));
                        controlsites_photostim_frames_near = cell2mat(controlsites_photostim_frames(idx_controlsites_near));
                        controlsites_photostim_frames_near = unique(ceil(controlsites_photostim_frames_near./bin_size_in_frame));

                        
                        dx = othersites_center_x - R_x(i_r);
                        dy = othersites_center_y - R_y(i_r);
                        distance2D = sqrt(dx.^2 + dy.^2)*pix2dist; %
                        idx_othersites_near = distance2D<=distance_to_exclude_other   & ~ (othersites_num== group_list(i_g));
                        othersites_photostim_frames_near = cell2mat(othersites_photostim_frames(idx_othersites_near));
                        othersites_photostim_frames_near = unique(ceil(othersites_photostim_frames_near./bin_size_in_frame));

                        
                       baseline_frames_clean=controlsites_photostim_frames_far(~ismember(controlsites_photostim_frames_far, [othersites_photostim_frames_near]));
                       baseline_frames_clean=baseline_frames_clean(~ismember(baseline_frames_clean, [target_photostim_frames]));
                       ...
                       target_photostim_frames_clean=target_photostim_frames(~ismember(target_photostim_frames, [othersites_photostim_frames_near]));
                       target_photostim_frames_clean=target_photostim_frames_clean(~ismember(target_photostim_frames_clean, [controlsites_photostim_frames_near]));
...
%                                 insert(STIM.ROIInfluenceRefinedTrace, k_trace);
%                             end
%                         end
%                     end
