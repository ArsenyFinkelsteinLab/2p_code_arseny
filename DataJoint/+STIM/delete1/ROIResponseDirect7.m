...
classdef ROIResponseDirect7 < dj.Imported
    properties
        %         keySource = IMG.PhotostimGroup;
        keySource = EXP2.SessionEpoch & 'flag_photostim_epoch =1' & (IMG.FOV & STIM.ROIInfluence7);
    end
    methods(Access=protected)
        function makeTuples(self, key)
            distance_to_closest_neuron=25; %  in microns, max distance to direct neuron
            
            rel_data = STIM.ROIInfluence7;
            ...
            for i_g = 1:1:numel(group_num)
                    ...
                    if ~isempty(C)
                        if C(1).response_p_value1_odd<=p_value & C(1).response_mean_odd>0 % checking if the neuron most close to the photostimulation coordinate has a significant and positive response
                            DIRECT=C(1);
                            if numel(C)>=2
                            end
                            ...
                            C_remaining=C;
                            C_remaining = C_remaining([C.response_p_value1_odd]<=p_value & [C.response_mean_odd]>0);
