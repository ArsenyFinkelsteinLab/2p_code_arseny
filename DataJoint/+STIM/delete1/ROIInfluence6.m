...
classdef ROIInfluence6 < dj.Computed
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            distance_to_exclude_all=75; %microns
            ...
            key_session = (EXP2.Session & key);
            key_spont = EXP2.SessionEpoch & 'session_epoch_type = "spont_only"' & key_session;
            key_spont  = fetch(key_spont);
            key_spont  = key_spont(1);

            
            rel_data_spont = (IMG.ROISpikes -IMG.ROIBad)  & key_spont;
            ...
            try
                F_original_photo = fetchn(rel_data ,'dff_trace','ORDER BY roi_number');
                F_original_spont = fetchn(rel_data_spont ,'dff_trace','ORDER BY roi_number');
            catch
                F_original_photo = fetchn(rel_data ,'spikes_trace','ORDER BY roi_number');
                F_original_spont = fetchn(rel_data_spont ,'spikes_trace','ORDER BY roi_number');
            end
            F_original_photo=cell2mat(F_original_photo);
            F_original_spont=cell2mat(F_original_spont);

            F_combined = [F_original_photo, F_original_spont];
            ...
            F_combined=zscore(F_combined,[],2);
            %             [U,S,V]=svd(F_binned); % S time X neurons; % U time X time;  V neurons x neurons
            
            F=F_combined(:,1:1:size(F_original_photo,2));
            
            F_baseline = F_combined(:,size(F_original_photo,2)+1:1:end);
            F_baseline=mean(F_baseline,1);
...
%                         baseline_frames_clean=allsites_photostim_frames_far(~ismember(allsites_photostim_frames_far, [allsites_photostim_frames_near-3, allsites_photostim_frames_near-2, allsites_photostim_frames_near-1, allsites_photostim_frames_near, allsites_photostim_frames_near+1]));
%                         baseline_frames_clean=baseline_frames_clean(~ismember(baseline_frames_clean, [target_photostim_frames-3, target_photostim_frames-2, target_photostim_frames-1, target_photostim_frames, target_photostim_frames+1]));
...
%                         single(distance2D*pix2dist)
                        ...
                        [StimStat, ~] = fn_compute_photostim_delta_influence_fixed_baseline (f_trace, target_photostim_frames_clean,F_baseline(i_r), timewind_response, time);
