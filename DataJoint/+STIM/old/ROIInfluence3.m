...
response_p_value1                   : double               # significance of response to photostimulation, relative to distant pre stimulus baseline, ttest
response_p_value2                   : double               # significance of response to photostimulation, relative to distant pre stimulus baseline, wilcoxon-ranksum
response_mean_odd                   : float                # for the  odd trials
response_p_value1_odd               : double                #
response_p_value2_odd               : double                #
response_mean_even                  : float                # for the  even trials
response_p_value1_even              : float                #
response_p_value2_even              : float                #

response_min_odd                    : float                # min of the trace during response window, odd trials
response_max_odd                    : float                # max of the trace during response window, odd trials
non_response_min_odd                : float                # min of the trace outside of the response window, odd trials
non_response_max_odd                : float                # max of the trace outside of the response window, odd trials
...
classdef ROIInfluence3 < dj.Computed
    properties
        ...
        keySource = (EXP2.SessionEpoch & 'flag_photostim_epoch =1' & IMG.FOVEpoch) & (STIMANAL.SessionEpochsIncluded& 'stimpower=150' & 'flag_include=1') & IMG.Volumetric
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            p_val=[0.0001, 0.001, 0.01, 0.05, 1]; % for saving  trace
            time_bin=0.5; %s
            ...
            %             group_list = fetchn((IMG.PhotostimGroup & key & (STIMANAL.NeuronOrControl & 'neurons_or_control=1' & 'num_targets>=30') ),'photostim_group_num','ORDER BY photostim_group_num');
            ...
            R_x = fetchn(rel_roi ,'roi_centroid_x','ORDER BY roi_number');
            R_y = fetchn(rel_roi ,'roi_centroid_y','ORDER BY roi_number');
            ...
            allsites_photostim_frames = unique(floor(cell2mat(allsites_photostim_frames)./bin_size_in_frame));
            ...
            for i_c = 1:1:numel(num_svd_components_removed_vector)
                ...
                parfor i_g = 1:1:numel(group_list)
                    ...
                    for i_r= 1:1:numel(roi_list)
                        ...
                        baseline_frames_clean=allsites_photostim_frames(~ismember(allsites_photostim_frames,[target_photostim_frames-1, target_photostim_frames, target_photostim_frames+1]));
                        ...
                        target_photostim_frames_clean=target_photostim_frames(~ismember(target_photostim_frames, [target_photostim_frames-1, target_photostim_frames+1]));
                        ...
                        k_response(i_r).non_response_min_odd = StimStat.non_response_min_odd;
                        k_response(i_r).non_response_max_odd = StimStat.non_response_max_odd;
                        k_response(i_r).response_min_odd = StimStat.response_min_odd;
                        k_response(i_r).response_max_odd = StimStat.response_max_odd;
                    ...
                    flag_prominent=[0,1]; %0, we take all responses matching the above criteria, 1 we take only prominent responses, for which the min or the max value of the response is larger than the largest fluctuation outside of the response window
                    
                    for i_dist = 1:1:numel(flag_distance)
                        for i_p=1:1:numel(p_val)
                            for i_sign = 1:1:numel(response_sign)
                                for i_prom = 1:1:numel(flag_prominent)
                                    ...
                                    if flag_prominent(i_prom)==1  % we take only prominent responses, for which the min or the max value of the response is larger than the largest fluctuation outside of the response window
                                        idx_rois = idx_rois & ([k_response.response_min_odd] < [k_response.non_response_min_odd] | [k_response.response_max_odd] > [k_response.non_response_max_odd]);
%                                         if sum(idx_rois)>0
%                                             sum(idx_rois)
%                                         end
                                    end
                                    ...
                                    k_trace.flag_prominent = flag_prominent(i_prom);
                                    
                                    k_trace.num_pairs = sum(idx_rois);
                                    insert(STIM.ROIInfluenceTrace3, k_trace);
                                end
                            end
                        end
                    end
