...
response_mean_1half                   : float                # for the  1half trials
response_p_value1_1half=null               : float                #
response_mean_2half=null                  : float                # for the  2half trials
response_p_value1_2half=null              : float               #
p_value_1half_vs_2half=null              : float               # correlation of the response between first and second half of the stimulation trials
...
classdef ROIInfluenceStability < dj.Computed
    properties
            ...
            for i_c = 1:1:numel(num_svd_components_removed_vector)
                ...
                parfor i_g = 1:1:numel(group_list) %parfor
                    ...
                    for i_r= 1:1:numel(roi_list)
                        ...
                        [StimStat] = fn_compute_photostim_delta_influence5_1st2nd_half (f_trace, target_photostim_frames_clean,baseline_frames_clean, timewind_response, time);
...
%                         end
                        ...
                        k_response(i_r).p_value_1half_vs_2half =StimStat.p_value_1half_vs_2half;
                    ...
                    %
                    %                                 idx_rois= [k_response.response_p_value1_1half]<=p_val(i_p)  & [k_response.response_distance_lateral_um]>=flag_distance_edge1(i_dist) & [k_response.response_distance_lateral_um]<flag_distance_edge2(i_dist);
                    ...
                    %                                     case 'excited'
                    %                                         idx_rois = idx_rois & [k_response.response_mean_1half]>0;
                    %                                     case 'inhibited'
                    %                                         idx_rois = idx_rois & [k_response.response_mean_1half]<0;
                    %                                 end
                    %                                 if sum(idx_rois)>1
                    %                                     k_trace.response_trace_mean = sum(StimTrace.response_trace_mean(idx_rois,:),1);
                    %                                     k_trace.response_trace_mean_1half = sum(StimTrace.response_trace_mean_1half(idx_rois,:),1);
                    %                                     k_trace.response_trace_mean_2half = sum(StimTrace.response_trace_mean_2half(idx_rois,:),1);
