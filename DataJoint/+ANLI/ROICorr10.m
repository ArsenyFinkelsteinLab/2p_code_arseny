...
classdef ROICorr10 < dj.Computed
    properties
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            dir_current_fig = [dir_base  '\Graph_correlation1_remove10PC\'];
            ...
            roi_number_list=fetchn(rel,'roi_number','ORDER BY roi_number');
            pos_x=fetchn(rel,'roi_centroid_x','ORDER BY roi_number')*pix2dist;
            pos_y=fetchn(rel,'roi_centroid_y','ORDER BY roi_number')*pix2dist;
            
            F = fetchn(IMG.ROIdeltaF & rel & key,'dff_trace','ORDER BY roi_number');
            F=cell2mat(F);
            for i = 1:1:size(F,1)
                f=F(i,:);
            ...
            [coeff,score,~, ~, explained] = pca(Fz);
            
            
            % Fz_reconst=score(:,:)*coeff';
            ...
            distance_bins_all=[prctile(all_distance,[0:12.5:100])];
            distance_bins_positive=[prctile(all_distance_positive,[0:12.5:100])];
            distance_bins_negative=[prctile(all_distance_negative,[0:12.5:100])];
