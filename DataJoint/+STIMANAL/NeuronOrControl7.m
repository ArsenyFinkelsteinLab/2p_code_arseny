...
classdef NeuronOrControl7 < dj.Imported
    properties
        keySource = EXP2.SessionEpoch & 'flag_photostim_epoch =1' & IMG.ROI & STIM.ROIResponseDirect7;
    end
    methods(Access=protected)
        function makeTuples(self, key)
            neurons_or_control = [1,0];
            rel = STIM.ROIResponseDirect7 & key;
            
            pval_theshold_neurons=0.05;
            pval_theshold_control=0.05;
            z_score_threshold_neurons =0;
            z_score_threshold_controls =100;
            minimal_num_of_target_trials_used=20;
            ...
            for i_n = 1:1:numel(neurons_or_control)
                ...
                if neurons_or_control(i_n) == 1 %neurons
                    ...
                    rel_direct= (rel & ['response_p_value1_odd<=' num2str(pval_theshold_neurons)] & ['response_mean_odd>' num2str(z_score_threshold_neurons)] &  ['num_of_target_trials_used>=' num2str(minimal_num_of_target_trials_used)]);
                elseif neurons_or_control(i_n) == 0 % control sites
                    ...
                    rel_direct= (rel & ['response_p_value1_odd>' num2str(pval_theshold_control)] & ['response_mean_odd<' num2str(z_score_threshold_controls)] &  ['num_of_target_trials_used>=' num2str(minimal_num_of_target_trials_used)]) ;
                end
