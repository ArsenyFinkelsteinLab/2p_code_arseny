...
classdef InfluenceVsCorrAngle < dj.Computed
    properties
...
%             bins_influence = [-inf,linspace(-0.15,0.15,6),inf];
            ...
            dir_fig = [dir_base  '\Photostim\Connectivity_vs_Tuning\single_sessions\tuning_by_preferred_angle'];
            session_date = fetch1(EXP2.Session & key,'session_date');
            
            rel_data_corr =STIMANAL.Target2AllCorrAngle & rel_include;
            ...
            rel_roi = (IMG.ROI & rel_include) & key;
...
%                 rel_target_signif_by_psth = (STIM.ROIResponseDirect & 'flag_zscore=1' &  rel_target) &    (IMG.ROI &  (LICK2D.ROILick2DangleStatsSpikes3bins & k_psth & 'theta_tuning_odd_even_corr>=0.5'));
%                 rel_target_signif_by_psth = (STIM.ROIResponseDirect  &  rel_target) &    (IMG.ROI &  (LICK2D.ROILick2DangleStatsSpikes3bins & k_psth & 'rayleigh_length>=0.05'));
                rel_target_signif_by_psth = (STIM.ROIResponseDirect2  &  rel_target) &    (IMG.ROI &  (LICK2D.ROILick2DangleStatsSpikes3bins & k_psth & 'theta_tuning_odd_even_corr_regular>=0.5'));
...
%                     roi_psth_signif = fetchn(LICK2D.ROILick2DangleStatsSpikes3bins & k_psth & rel_roi & 'theta_tuning_odd_even_corr>=0.5', 'roi_number', 'ORDER BY roi_number');
%                     roi_psth_signif = fetchn(LICK2D.ROILick2DangleStatsSpikes3bins & k_psth & rel_roi & 'rayleigh_length>=0.05', 'roi_number', 'ORDER BY roi_number');
                    roi_psth_signif = fetchn(LICK2D.ROILick2DangleStatsSpikes3bins & k_psth & rel_roi & 'theta_tuning_odd_even_corr_regular>=0.5', 'roi_number', 'ORDER BY roi_number');

                    roi_psth_all = fetchn(LICK2D.ROILick2DangleStatsSpikes3bins & k_psth & rel_roi, 'roi_number', 'ORDER BY roi_number');
