...
classdef OutDegreeETL < dj.Imported
    properties
        keySource = EXP2.SessionEpoch & STIM.ROIResponseDirect;
    end
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            rel_group = IMG.PhotostimGroup &   STIM.ROIResponseDirect & key;
            photostim_group = fetch(rel_group,'ORDER BY photostim_group_num');
            
            key_ROI=fetch(STIM.ROIResponseDirect & key,'ORDER BY photostim_group_num');
            ...
            parfor i_d = 1:1:numel(max_distance_lateral_vec)
                for i_p = 1:1:numel(p_val)
                    ...
                    for i_g = 1:1: rel_group.count
                        ...
                        rel = (STIM.ROIInfluence & rel_group_current) & sprintf('response_p_value1<=%.4f',current_p_val) & sprintf('response_distance_lateral_um>%d',minimal_lateral_distance)  & sprintf('response_distance_lateral_um<=%d',(max_distance_lateral_vec(i_d)));
                        rel_current = rel ;
