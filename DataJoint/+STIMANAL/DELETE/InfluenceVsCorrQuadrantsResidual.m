...
classdef InfluenceVsCorrQuadrantsResidual < dj.Computed
    properties
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            dir_fig = [dir_base  '\Photostim\influence_vs_corr\corr_quadrants\residuals\'];
            ...
            for i_n = 1:1:numel(neurons_or_control_flag)
                ...
                rel_target_signif_by_psth = (STIM.ROIResponseDirect5 &  rel_target) &    (IMG.ROI &  (LICK2D.ROILick2DQuadrants & k_psth & 'psth_odd_even_corr>=0.5'));
                ...
                idx_include(Data_distance_lateral>maximal_distance  )=false; %exlude all cells further than maximal distance from target
                ...
                for i_p=1:1:numel(p_val)
                    ...
                    roi_psth_signif = fetchn(LICK2D.ROILick2DQuadrants & k_psth & rel_roi & 'psth_odd_even_corr>=0.5', 'roi_number', 'ORDER BY roi_number');
                    roi_psth_all = fetchn(LICK2D.ROILick2DQuadrants & k_psth & rel_roi, 'roi_number', 'ORDER BY roi_number');
                    ...
                    %
                    Data_distance_axial =Data_distance_axial(idx_group_list_signif,idx_psth_signif);
                    Data_distance_lateral =Data_distance_lateral(idx_group_list_signif,idx_psth_signif);
                    ...
                    for i_l=1:1:numel(distance_lateral_bins)-1
                        ix_l = xx>=distance_lateral_bins(i_l) & xx<distance_lateral_bins(i_l+1);
                        for i_a = 1:1:numel(distance_axial_bins)
                            ix_a = yy==distance_axial_bins(i_a);
                            ix_bin = ix_l & ix_a;
                            data_in_bin = dd(ix_bin);
                    ...
                    DataCorr=DataCorr(:);
                            ...
                            for i_l=1:1:numel(distance_lateral_bins)-1
                                ...
                                for i_a = 1:1:numel(distance_axial_bins)
                                    ...
                                    ix_bin = ix_l & ix_a;
                                    DataCorr(ix_bin) =DataCorr(ix_bin) -  MeanCorrDistance_binned(i_a, i_l);
                                end
                            end
                    
                    
%                     for i_g = 1:1:numel(group_list)
%                         current_DataCorr = DataCorr(i_g,:);
%                         current_distance_lateral= Data_distance_lateral(i_g,:);
%                         current_distance_axial= Data_distance_axial(i_g,:);
%                         
...
%                                 ix_bin = ix_l & ix_a;
%                                 data_in_bin = current_DataCorr(ix_bin);
%                                 current_DataCorr(ix_bin) =data_in_bin - MeanCorrDistance_binned(i_a, i_l);
%                             end
%                         end
%                         DataCorr(i_g,:)=current_DataCorr;
%                     end
