...
classdef ROIClusterCorrThresholded < dj.Computed
    properties
...
%             threshold_for_event_vector = [0.1, 0.25, 0.5, 1];
            threshold_for_event_vector = [0.1, 0.25];

            dir_base =fetch1(IMG.Parameters & 'parameter_name="dir_root_save"', 'parameter_value');
            dir_save_figure_original = [dir_base 'Lick2D\Clusters\CorrThresholded\'];
%             smooth_window=1;
...
%             Fall=movmean(Fall,[smooth_window smooth_window],2,'omitnan','Endpoints','shrink');
            
            
            
            for i_th = 1:1:numel(threshold_for_event_vector)
                threshold= threshold_for_event_vector(i_th);
                dir_save_figure = [dir_save_figure_original sprintf('Threshold%.2f',threshold) '\'];
                Fall_thresholded=Fall;
                Fall_thresholded(Fall<=threshold)=0;
                %             Fall(Fall>threshold)=1;
                [rho,~]=corr(Fall_thresholded');
                rho(isnan(rho(:)))=0;
