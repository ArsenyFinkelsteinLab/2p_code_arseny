...
classdef ROICorrLocalPhoto3Temp < dj.Computed
    properties
        keySource = EXP2.SessionEpoch  & IMG.ROI & IMG.ROISpikes & (EXP2.Session & STIM.ROIResponseDirect3) & IMG.Volumetric - IMG.Mesoscope;
    end
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            rel_roi_xy = (IMG.ROIPositionETL2-IMG.ROIBad)  & key; % to correct for ETL abberations

            %             radius_size_vector=[25,50,75,100, 125, 150, 175, 200];
%             radius_size_vector=[50,100,150, 200];
            radius_size_vector=[50,150,200];
            ...
            for i_c = 1:1:numel(num_svd_components_removed_vector)
                ...
                for ir = 1:1:numel(radius_size_vector)
                    fn_local_correlations_space(key, key_ROI,F,x_all, y_all, radius_size_vector(ir), POP.ROICorrLocalPhoto3, min_lateral_distance )
                end
            end
        end
    end
