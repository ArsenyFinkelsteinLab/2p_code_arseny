...
classdef ROIClusterCorrNeuropil < dj.Computed
    properties
        
        keySource = (EXP2.SessionEpoch & IMG.ROIdeltaFNeuropil & IMG.Mesoscope)-EXP2.SessionEpochSomatotopy;
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            dir_save_figure = [dir_base '\Lick2D\Clusters\CorrNeuropil\'];
...
%              n_clust_vector = [20, 100, 500];
            ...
            roi_list=fetchn(IMG.ROIdeltaFNeuropil & key,'roi_number','ORDER BY roi_number');
            chunk_size=500;
            for i_chunk=1:chunk_size:numel(roi_list)
                ...
                temp_Fall=cell2mat(fetchn(IMG.ROIdeltaFNeuropil & key & sprintf('roi_number>=%d',roi_interval(1)) & sprintf('roi_number<%d',roi_interval(2)),'dff_trace','ORDER BY roi_number'));
                temp_roi_num=fetchn(IMG.ROIdeltaFNeuropil & key & sprintf('roi_number>=%d',roi_interval(1)) & sprintf('roi_number<%d',roi_interval(2)),'roi_number','ORDER BY roi_number');
            ...
            key_ROIs= fetch(IMG.ROIdeltaFNeuropil & key, 'ORDER BY roi_number');
