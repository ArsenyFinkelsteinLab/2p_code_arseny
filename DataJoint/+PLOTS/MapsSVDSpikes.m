...
classdef MapsSVDSpikes < dj.Computed
    properties
        keySource = EXP2.SessionEpoch & POP.ROISVDSpikes  & IMG.Mesoscope & POP.ROISVDDistanceScaleSpikes;
    end
    methods(Access=protected)
        function makeTuples(self, key)
            
            dir_base = fetch1(IMG.Parameters & 'parameter_name="dir_root_save"', 'parameter_value');
            dir_current_fig = [dir_base  '\Lick2D\brain_maps\SVD\spikes\'];
            
            rel_roi = (IMG.ROI - IMG.ROIBad) & key;
            rel_data = (POP.ROISVDSpikes & 'threshold_for_event=0' & 'time_bin=1.5') & rel_roi & key;
            rel_data_spatial_scale = POP.ROISVDDistanceScaleSpikes  & key;
