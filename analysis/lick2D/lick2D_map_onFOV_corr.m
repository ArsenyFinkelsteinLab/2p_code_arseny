function lick2D_map_onFOV_corr()
close all;
...
dir_current_fig = [dir_base  '\Lick2D\population\onFOV_corr_distance\bins' num2str(key.number_of_bins) '\'];
if flag_all_or_signif ==2
    rel= (ANLI.ROILick2Dmap * ANLI.ROILick2Dangle * ANLI.ROILick2DangleShuffle *  ANLI.ROILick2DmapShuffle * ANLI.ROILick2DPSTH)* IMG.ROI*IMG.ROIdepth  & IMG.ROIGood & key   & 'lickmap_odd_even_corr>-1' & 'rayleigh_length>=0' & 'theta_tuning_odd_even_corr>-1' & 'pval_rayleigh_length>=0';
...
lateral_distance_bins = [0:10:150,inf]; % in microns
eucledian_distance_bins = [10:10:150];
...
r = corr(M.psth_averaged_over_all_positions');
r = r(idx_not_self);
dtheta=r;
%% Lateral distance
...
idx = (bin ==i);
    dtheta_XYdist_mean(i) = (mean((dtheta(idx))));
    dtheta_XYdist_var(i) = (std((dtheta(idx))))/sqrt(sum(idx));
end
...
for i=1:1:numel(lateral_distance_bins)-1
idx = (bin ==i);
    dtheta_XYdist_shuffled(i) = (mean((dtheta_shuffled(idx))));
end
...
idx = (bin ==i);
    dtheta_Zdist_mean(i) = (mean((dtheta_column(idx))));
    dtheta_Zdist_var(i) = (std((dtheta_column(idx))))/sqrt(sum(idx));
end
...
for i=1:1:numel(axial_distance_bins)-1
idx = (bin ==i);
    dtheta_Zdist_mean_shuffled(i) = (mean((dtheta_column_shuffled(idx))));
...
% eucledian_distance_bins = lateral_distance_bins(2:end);
[N,~,bin] = histcounts(d3D(:),eucledian_distance_bins);
for i=1:1:numel(eucledian_distance_bins)-1
idx = (bin ==i);
    dtheta_3Ddist_mean(i) = (mean((dtheta(idx))));
    dtheta_3Ddist_var(i) = (std((dtheta(idx))))/sqrt(sum(idx));
end
...
idx = (bin ==i);
dtheta_shuffled = dtheta(idx_shuffled);
    dtheta_3Ddist_shuffled(i) = (mean((dtheta_shuffled(idx))));
end

ax1=axes('position',[position_x2(5), position_y2(2), panel_width2, panel_height2]);
hold on;
plot(eucledian_distance_bins(1:1:end-1),dtheta_3Ddist_mean,'-r')
plot(eucledian_distance_bins(1:1:end-1),dtheta_3Ddist_shuffled,'-k')
ylim([-0.1,1]);
xlabel('Eucledian (3D) distance (\mum)');
ylabel('\Delta\theta (\circ)');
title(sprintf('Preferred Direction \nEucledian (3D)  distance'));
xlim([0,eucledian_distance_bins(end-1)]);
% set(gca,'YTick',[-1 0 1]);
