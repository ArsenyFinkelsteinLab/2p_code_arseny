function    lick2D_video_yaw_learning()
clf
...
%     clf;
    ...
    hold on
    plot(sessions_list,yaw_after_mean,'.-b')
    plot(sessions_list,yaw_before_mean,'.-','Color',[0.5, 0.5,0.5])
    xlabel('Session');
    ylabel('Mean |theta| (deg)');
    ylim([0, ceil(max([yaw_after_mean,yaw_before_mean]))]);
