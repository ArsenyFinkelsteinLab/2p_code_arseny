function [start_file, end_file, lick_tr_times_relative_to_first_lick,lick_tr_total ] = fn_parse_into_trials_and_get_lickrate (key, frame_rate, time_bin)
...
lick_tr_times_relative_to_first_lick =[];
lick_tr_total =[];
...
for i_tr = 1:1:numel(trial_num)
    ...
    if ~isempty(licks)
        ...
        lick_tr_times_relative_to_first_lick{i_tr}=licks-licks(1);
        lick_tr_total(i_tr)=sum((licks-licks(1))>=time_bin(1) & (licks-licks(1))<=time_bin(end));
