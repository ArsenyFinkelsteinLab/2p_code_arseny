function lick2D_local_corr_map(key, radius_size, dir_current_fig)
close all;
prctile_threshold = 100; %90
...
rel = IMG.ROI* POP.ROICorrLocal*IMG.PlaneCoordinates & key;
...
corr_local=M.corr_local;
bins = [-inf,linspace(min(corr_local), prctile(corr_local,prctile_threshold),1000),inf];
...
title(sprintf('anm %d session %d \n %s,  Corr radius=%d \n Total planes = %d ', key.subject_id, key.session,  session_epoch_label,key.radius_size,  number_of_planes));



filename = [filename session_epoch_label '_radius' num2str(key.radius_size)];
