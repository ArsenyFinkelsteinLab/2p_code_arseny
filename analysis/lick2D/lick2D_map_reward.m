...
if flag_all_or_signif ==1
    ...
    rel= ANLI.ROILick2DmapReward * ANLI.ROILick2Dangle * ANLI.ROILick2DangleShuffle * ANLI.ROILick2DmapShuffle * ANLI.ROILick2DPSTHReward & IMG.ROIGood & key;
elseif flag_all_or_signif ==2
    dir_current_fig = [dir_base  '\Lick2D\anm' num2str(key.subject_id) '\' session_date '\cells_signif_2Dlick\'];
    rel= (ANLI.ROILick2DmapReward * ANLI.ROILick2Dangle * ANLI.ROILick2DangleShuffle * ANLI.ROILick2DmapShuffle * ANLI.ROILick2DPSTHReward) & IMG.ROIGood & key  & 'pval_information_per_spike<=0.001';
elseif flag_all_or_signif ==3
    dir_current_fig = [dir_base  '\Lick2D\anm' num2str(key.subject_id) '\' session_date '\cells_signif_motor\' ];
    rel= (ANLI.ROILick2DmapReward * ANLI.ROILick2Dangle * ANLI.ROILick2DangleShuffle * ANLI.ROILick2DmapShuffle * ANLI.ROILick2DPSTHReward) & IMG.ROIGood & key  & 'pval_psth<=0.05';
...
dir_current_fig = [dir_current_fig 'supersignif\' 'time' num2str(key.fr_interval_start) '-' num2str(key.fr_interval_end) '\' 'bins' num2str(key.number_of_bins) '\' 'reward\'];
...
for i_roi=1:1:size(M,1)
    ...
    hold on;
    psth_m_regular= M(i_roi).psth_averaged_over_all_positions_regular;
    psth_m_large= M(i_roi).psth_averaged_over_all_positions_large;
    psth_m_small= M(i_roi).psth_averaged_over_all_positions_small;
    
    plot(time,psth_m_regular,'-r');
    plot(time,psth_m_large,'-b');
    plot(time,psth_m_small,'-g');
    ...
    xlim(time_bin);
    ylim([0, ceil(nanmax([psth_m_regular,psth_m_small,psth_m_large]))]);
    ...
    set(gca,'XTick',[-2,0,2],'Ytick',[0, ceil(nanmax([psth_m_regular,psth_m_small,psth_m_large]))],'TickLength',[0.05,0], 'FontSize',10);
