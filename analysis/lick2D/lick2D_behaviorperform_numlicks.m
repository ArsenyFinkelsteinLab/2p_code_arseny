function lick2D_behaviorperform_numlicks()
close all;
...
B=fetch(rel, '*', 'ORDER BY trial');


rel_licks = TRACKING.VideoNthLickTrial& key & 'lick_number_relative_to_firsttouch>=0';
% rel_licks = TRACKING.VideoNthLickTrial& key';
...
parfor i_tr=1:1:numel(B)
    num_licks_in_tr(i_tr) =  sum([L.trial]==i_tr);
end
...
for i_x=1:1:numel(x_bins_centers)
    for i_z=1:1:numel(z_bins_centers)
        ...
        num_licks(i_z,i_x) = sum(num_licks_in_tr(idx));
...
subplot(2,2,2)
num_licks_per_trial = num_licks./num_response;
%     axes('position',[position_x2(4),position_y2(1)-0.03, panel_width2*1.5, panel_height2*1.5])
imagescnan(x_bins_centers, z_bins_centers, num_licks_per_trial)
max_map=max(num_licks_per_trial(:));
caxis([0 max_map]); % Scale the lowest value (deep blue) to 0
colormap(parula)
%     title(sprintf('ROI %d anm%d %s\n \n Positional (2D) tuning  \n I = %.2f bits/spike  \n  p-val = %.4f  ',roi_number(i_roi),key.subject_id, session_date, M(i_roi).information_per_spike, M(i_roi).pval_information_per_spike ), 'FontSize',10);
axis xy
...
set(gca,'YDir','normal');
set(gca, 'FontSize',10);
title('number licks/trial after touch');
