function lick2D_map_onFOV_clusters_corr_meso3(key, flag_plot_clusters_individually, n_clust, M_all)
close all;
...
filename=[];
dir_current_fig = [dir_base  '\Lick2D\brain_maps\clusters_corrDiscrete\'];
rel = IMG.ROI &  ((POP.ROIClusterCorrDiscrete& key) & 'heirar_cluster_percent>=1')  & IMG.ROIGood ;


rel1 = ((IMG.ROI*POP.ROIClusterCorrDiscrete*IMG.PlaneCoordinates) & IMG.ROIGood & key) & 'heirar_cluster_percent>=1';
...
filename = [filename 'anm' num2str(key.subject_id) '_s' num2str(key.session) '_' session_date '_' session_epoch_label '_nsplits' num2str(n_clust)]
...
for i_clus = 1:1:min([numel(unique_cluster_id),10])
    ...
    title(sprintf('%s \n anm%d session %d,  Total planes = %d\nsplits = %d  %.1f%% cells shown', session_epoch_label, key.subject_id,key.session, number_of_planes, n_clust, percent_clustered));
