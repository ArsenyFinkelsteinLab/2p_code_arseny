function lick2D_map_onFOV_clusters_shapeandselectivity()
close all;
...
filename=[];
dir_current_fig = [dir_base  '\Lick2D\population\onFOV_clusters_shapeandselectivity\bins' num2str(key.number_of_bins) '\'];
if flag_all_or_signif ==1
    rel= (ANLI.ROILick2Dmap * ANLI.ROILick2DPSTH * ANLI.ROILick2Dselectivity * ANLI.ROIHierarClusterShapeAndSelectivity )* IMG.ROI  & IMG.ROIGood & key ;
elseif flag_all_or_signif ==2
    rel= (ANLI.ROILick2Dmap * ANLI.ROILick2DPSTH  * ANLI.ROILick2Dselectivity * ANLI.ROIHierarClusterShapeAndSelectivity )* IMG.ROI  & IMG.ROIGood & key * IMG.ROI  & IMG.ROIGood & key   & 'lickmap_odd_even_corr>0.5';
...
% All planes
ax1=axes('position',[position_x1(3), position_y1(3), panel_width1*2, panel_height1*2]);
...
my_colormap(3,:)=[1 1 0];
my_colormap(4,:)=[0 1 0];
my_colormap(5,:)=my_colormap(4,:);
my_colormap(9,:)=my_colormap(4,:);
my_colormap(6,:)=[0 1 1];
my_colormap(8,:)=[1 0 1];

for i_roi=1:1:size(M,1)
    plot(M.roi_centroid_x(i_roi)*pix2dist, M.roi_centroid_y(i_roi)*pix2dist,'.','Color',my_colormap(roi_cluster_renumbered(i_roi),:),'MarkerSize',15)
end
axis xy
...
if flag_plot_rois==1
    ...
    for i_roi=1:1:size(MPlane,1)
        plot(MPlane.roi_centroid_x(i_roi)*pix2dist, MPlane.roi_centroid_y(i_roi)*pix2dist,'.','Color',my_colormap(roi_cluster_renumbered(i_roi),:),'MarkerSize',5)
    end
end
...
PSTH2 = PSTH2./nanmax(PSTH1_before_scaling,[],2);
...
for ic=1:1:numel(unique_cluster_id)
    ...
    axes('position',[position_x2(ic), position_y2(ic)-0.15, panel_width2, panel_height2]);
    hold on;
    plot([0 0],[0,1],'-k');
    plot(M.time_psth(1,:),psth_cluster(ic,:))
