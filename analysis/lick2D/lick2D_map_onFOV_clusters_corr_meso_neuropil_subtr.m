function lick2D_map_onFOV_clusters_corr_meso_neuropil_subtr(key, flag_plot_clusters_individually, n_clust, M_all, dir_current_fig)
close all;
...
filename=[];
rel = IMG.ROI &  ((POP.ROIClusterCorr& key) & 'heirar_cluster_percent>=1')  & IMG.ROIGood ;


rel1 = ((IMG.ROI*POP.ROIClusterCorrNeuropilSubtr*IMG.PlaneCoordinates) & IMG.ROIGood & key) & 'heirar_cluster_percent>=1';
...
filename = [filename 'anm' num2str(key.subject_id) '_s' num2str(key.session) '_' session_date '_' session_epoch_label num2str(key.session_epoch_number) '_nsplits' num2str(n_clust)]
...
for i_clus = 1:1:min([numel(unique_cluster_id),10])
    ...
    title(sprintf('Neuropil \n%s %d %s\n anm%d session %d,  Total planes = %d\nsplits = %d  %.1f%% cells shown', session_epoch_label, key.session_epoch_number,session_date, key.subject_id,key.session, number_of_planes, n_clust, percent_clustered));
