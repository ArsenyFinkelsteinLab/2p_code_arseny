function lick2D_map_onFOV_clusters_shape_meso(key, flag_plot_clusters_individually)
close all;
...
filename=[];
dir_current_fig = [dir_base  '\Lick2D\population\onFOV_clusters_shape_meso\bins' num2str(key.number_of_bins) '\'];
if flag_all_or_signif ==1
    rel= (ANLI.ROILick2DPSTH *IMG.PlaneCoordinates * ANLI.ROIHierarClusterShape )* IMG.ROI  & IMG.ROIGood & key ;
elseif flag_all_or_signif ==2
    ...
    rel= (IMG.PlaneCoordinates * ANLI.ROIHierarClusterShape )* IMG.ROI  & IMG.ROIGood & key  & (ANLI.ROILick2Dmap  & 'lickmap_odd_even_corr>=-1');
