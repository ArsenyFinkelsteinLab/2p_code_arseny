function fn_compute_predictors (key, self, time_bin)
...
for i_p=1:1:numel(predictor_name)
    key_insert(counter).trial_predictor = fn_predictors_move(key,time_bin, predictor_name{i_p}, tracking_start_time);
...
for i_p=1:1:numel(predictor_name)
    key_insert(counter).trial_predictor = fn_predictors_task(key,time_bin, predictor_name{i_p}, num_resampled_frames);
    key_insert(counter).predictor_name=predictor_name{i_p};
    counter=counter +1;
end
