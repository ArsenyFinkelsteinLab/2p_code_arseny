function lick2D_map_onFOV_clusters()
close all;
...
filename=[];
dir_current_fig = [dir_base  '\Lick2D\population\onFOV_clusters\bins' num2str(key.number_of_bins) '\'];
if flag_all_or_signif ==1
    rel= (ANLI.ROILick2Dmap * ANLI.ROILick2DPSTH * ANLI.ROIHierarCluster )* IMG.ROI  & IMG.ROIGood & key ;
elseif flag_all_or_signif ==2
    rel= (ANLI.ROILick2Dmap * ANLI.ROILick2DPSTH * ANLI.ROIHierarCluster )* IMG.ROI  & IMG.ROIGood & key * IMG.ROI  & IMG.ROIGood & key   & 'lickmap_odd_even_corr>0.5';
...
my_colormap(1,:)=my_colormap(4,:);

for i_roi=1:1:size(M,1)
    plot(M.roi_centroid_x(i_roi)*pix2dist, M.roi_centroid_y(i_roi)*pix2dist,'.','Color',my_colormap(roi_cluster_renumbered(i_roi),:),'MarkerSize',5)
end
axis xy
...
%% Plot clusters
PSTH= M.psth_averaged_over_all_positions;
