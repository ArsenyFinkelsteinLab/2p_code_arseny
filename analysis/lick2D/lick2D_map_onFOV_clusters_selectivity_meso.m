function lick2D_map_onFOV_clusters_selectivity_meso(key, flag_plot_clusters_individually)
close all;
...
filename=[];
dir_current_fig = [dir_base  '\Lick2D\population\onFOV_clusters_selectivity_meso\bins' num2str(key.number_of_bins) '\'];
if flag_all_or_signif ==1
    rel= (ANLI.ROILick2DPSTH * ANLI.ROILick2Dselectivity*IMG.PlaneCoordinates * ANLI.ROIHierarClusterSelectivity )* IMG.ROI  & IMG.ROIGood & key ;
elseif flag_all_or_signif ==2
    ...
    rel= (IMG.PlaneCoordinates * ANLI.ROIHierarClusterSelectivity )* IMG.ROI  & IMG.ROIGood & key  & (ANLI.ROILick2Dmap  & 'lickmap_odd_even_corr>=-1');
...
my_colormap(2,:)=[0 0 0];
my_colormap(3,:)=[1 0 0];
my_colormap(4,:)=[0.8 0.8 0];
my_colormap(5,:)=[0 0.7 0];
my_colormap(6,:)=[0 0 1];
my_colormap(7,:)=[0 0.8 0.8];
my_colormap(8,:)=[1 0 1];

my_colormap(100,:)=[0.93 0.93 0.93];
