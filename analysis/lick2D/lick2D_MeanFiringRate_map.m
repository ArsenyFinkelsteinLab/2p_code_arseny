...
dir_current_fig = [dir_base  '\Lick2D\brain_maps\mean_firing_rate\'];


rel = IMG.ROI* IMG.ROIdeltaFMean*IMG.PlaneCoordinates & key;
...
mean_dff=M.mean_dff;
bins = [-inf,linspace(prctile(mean_dff,5),prctile(mean_dff,95),100),inf];

cmap = jet(length(bins));
colormap (cmap);
mean_dff(isnan(mean_dff))=0;

[~,~,bins_idx]=histcounts(mean_dff,bins);
