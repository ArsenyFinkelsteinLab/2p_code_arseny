function PLOT_SVDVarianceShared()
close all
dir_base = fetch1(IMG.Parameters & 'parameter_name="dir_root_save"', 'parameter_value');
dir_current_fig = [dir_base  '\Photostim\photostim_distance\'];

rel_data2 = POP.ROISVDCrossValid  & (IMG.Mesoscope- IMG.Volumetric) & 'threshold_for_event=0' & 'time_bin=1.5';
...
for i=1:1:num_sessions
    num_components_behav(i) = numel(DATA(i).var_explained_test);
end
num_components_behav = min(num_components_behav);

for i=1:1:num_sessions
    v = DATA(i).var_explained_test;
    VE_behav (i,:) = v(1:1:num_components_behav);
end
...
num_sessions = numel(DATA);
for i=1:1:num_sessions
    num_components_spont(i) = numel(DATA(i).var_explained_test);
end
num_components_spont = min(num_components_spont);

for i=1:1:num_sessions
    v = DATA(i).var_explained_test;
    VE_spont (i,:) = v(1:1:num_components_spont);
end
