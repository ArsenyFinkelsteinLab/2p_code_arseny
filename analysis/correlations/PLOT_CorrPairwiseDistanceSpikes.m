function PLOT_CorrPairwiseDistanceSpikes()
close all;
...
rel_data = (POP.CorrPairwiseDistanceSVDSpikes-IMG.Volumetric) & IMG.Mesoscope;
...
for i_c=1:1:numel(num_svd_components_removed_vector)
    ...
    D=fetch(rel_data&key & (EXP2.Session & (EXP2.SessionEpoch & 'session_epoch_type="spont_only"') &  (EXP2.SessionEpoch & 'session_epoch_type="behav_only"')),'*');
    subplot(2,3,3)
    hold on
    if i_c==1
        plot([x(1),x(end)],[0,0],'-k');
