function fn_pv_corr_ensemble_extraction_avg_correlation_mps(k, date, dir_save_figure, clustering_option_name, rel, flag_update_cluster_group_table)
...
for i_condition = 1:1:(numel(trials)+1)
    PV_current=[];
    r =[];
    r_mat_trials=[];
    r_mat_cells=[];
    ...
    if i_condition<=numel(trials)
        
    PV_current = [PV_all(:,trials(i_condition).tr),PV_all(:,trials(i_condition).tr+size(PV.epoch{2},2))];
    ...
    % correlation matrix
    axes('position',[position_x1(i_condition) position_y1(2) panel_width1 panel_height1]);
    ...
    %colorbar
    if i_condition==1
        axes('position',[position_x1(i_condition)+0.09 position_y1(2)-0.05 panel_width1/3 panel_height1]);
        c=colorbar;
    ...
    axes('position',[position_x1(i_condition) position_y1(3) panel_width1 panel_height1]);
    ...
    axes('position',[position_x1(i_condition)+0.09 position_y1(3) panel_width1/3 panel_height1]);
    fn_plot_cluster_hist(cl_id_cells, cluster_mat_cells, order_cells, r_mat_cells);
    
    %colorbar
    if i_condition==1
        axes('position',[position_x1(i_condition)+0.09 position_y1(3)-0.05 panel_width1/3 panel_height1]);
        c=colorbar;
        set(c,'Location','southoutside');
        axis off;
        box off;
    ...
    % correlation matrix
    axes('position',[position_x1(i_condition) position_y1(4) panel_width1 panel_height1]);
    ...
    %colorbar
    if i_condition==1
        axes('position',[position_x1(i_condition)+0.09 position_y1(4)-0.05 panel_width1/3 panel_height1]);
        c=colorbar;
        set(c,'Location','southoutside');
        axis off;
        box off;
    ...
    % cluster pairwise distance
    axes('position',[position_x1(i_condition) position_y1(5) panel_width1 panel_height1]);
    [cl_id_trials, cluster_mat_trials, order_trials]=fn_ClusterPVtrials(r_mat_trials);
    fn_plot_cluster_mat(cluster_mat_trials);
    
    
    % cluster pairwise histogram
    axes('position',[position_x1(i_condition)+0.09 position_y1(5) panel_width1/3 panel_height1]);
    fn_plot_cluster_hist(cl_id_trials, cluster_mat_trials, order_trials, r_mat_trials);
    
    %colorbar
    if i_condition==1
        axes('position',[position_x1(i_condition)+0.09 position_y1(5)-0.05 panel_width1/3 panel_height1]);
        c=colorbar;
        set(c,'Location','southoutside');
        axis off;
        box off;
    ...
    axes('position',[position_x1(i_condition) position_y1(1) panel_width1 panel_height1]);
    ...
    r_mat_cells_conditions(i_condition,:,:) = r_mat_cells;
    
    
    if flag_update_cluster_group_table~=0 && i_condition>numel(trials)
