function PLOT_Network_Degree_vs_tuning_ETL4()
clf
...
rel_session = EXP2.Session & (STIMANAL.OutDegree3 & IMG.Volumetric) & (EXP2.SessionEpoch& 'session_epoch_type="spont_only"') & (STIMANAL.NeuronOrControlNumber3  & 'num_targets_neurons>=50') & (STIMANAL.SessionEpochsIncludedFinal & 'flag_include=1');
...
for i_s = 1:1:rel_session.count
    ...
    k_s = sessions(i_s);
    rel_degree = STIMANAL.OutDegree3 *STIM.ROIResponseDirect3  & (STIMANAL.NeuronOrControl3 & k_neurons_or_control) & k_degree & k_s &sprintf('p_val<=%.5f',p_val*1.1)  & sprintf('p_val>%.5f',p_val*0.9);
    ...
    if rel_signal1.count==0 % this happens if there is no behavioral epoch
        continue
    end
    
    
%     rel_signal2 = LICK2D.ROILick2DRewardStatsSpikes  & k_s & key_epoch;
%     rel_signal3 = LICK2D.ROILick2DContactenatedSpikes2 & k_s & key_epoch;
...
%     DATA_SIGNAL1 = struct2table(fetch(rel_signal1, 'goodness_of_fit_vmises'));
        DATA_SIGNAL1 =fetchn(rel_signal1, 'goodness_of_fit_vmises','ORDER BY roi_number');

%     DATA_SIGNAL2 = struct2table(fetch(rel_signal2, 'reward_mean_pval_regular_large'));
%     DATA_SIGNAL3 = fetchn(rel_signal3, 'psth_position_concat_regularreward_odd_even_corr_binwise','ORDER BY roi_number');
%     DATA_SIGNAL3 = fetchn(rel_signal3, 'psth_position_concat_regularreward_odd_even_corr_binwise','ORDER BY roi_number');
    DATA_SIGNAL3 = fetchn(rel_signal3, 'psth_odd_even_corr','ORDER BY roi_number');
