function [StimStat, StimTrace] = fn_compute_photostim_delta_influence_fixed_baseline (f_trace, photostim_start_frame,baseline_fixed, timewind_response,time)
...
delta_trace_trials = single(F -baseline_fixed);
...
StimStat.response_min_odd = min(mean(delta_trace_trials(idx_odd_trials,idx_response),1));
StimStat.response_max_odd = max(mean(delta_trace_trials(idx_odd_trials,idx_response),1));
StimStat.non_response_min_odd = min(mean(delta_trace_trials(idx_odd_trials,idx_not_response),1));
StimStat.non_response_max_odd = max(mean(delta_trace_trials(idx_odd_trials,idx_not_response),1));
...
% [StimStat.response_p_value2_odd] = ranksum(F_response_trials(idx_odd_trials),F_baseline_trials);
% [StimStat.response_p_value2_even] = ranksum(F_response_trials(idx_even_trials),F_baseline_trials);
...
StimStat.response_mean_over_std_odd = mean(delta_response_trials_odd)/std(f_trace);
[~, StimStat.response_p_value1_odd] = ttest(delta_response_trials(idx_odd_trials));
% [~, StimStat.response_p_value2_odd] = ttest(delta_baseline_trials1(idx_odd_trials));
% [~, StimStat.response_p_value3_odd] =ttest(delta_baseline_trials2(idx_odd_trials));

% Statistics even trials
StimStat.response_mean_even = mean(delta_response_trials_even);
StimStat.response_mean_over_std_even = mean(delta_response_trials_even)/std(f_trace);
[~, StimStat.response_p_value1_even] = ttest(delta_response_trials(idx_even_trials));
% [~, StimStat.response_p_value2_even] = ttest(delta_baseline_trials1(idx_even_trials));
% [~, StimStat.response_p_value3_even] = ttest(delta_baseline_trials2(idx_even_trials));
...
% hold on
% plot(time, 0,'-k')
% plot(time, mean(delta_trace_trials(idx_odd_trials,:),1),'-r')
% plot(time, mean(delta_trace_trials(idx_even_trials,:),1),'-b')
% 
% % if StimStat.response_p_value1_odd<=0.01 & (StimStat.response_min_odd<StimStat.non_response_min_odd || StimStat.response_max_odd>StimStat.non_response_max_odd)
% if    StimStat.response_p_value1_odd<=0.05 & StimStat.response_mean_odd>0 %& StimStat.response_p_value1<0.01 %&  (StimStat.response_min_odd<StimStat.non_response_min_odd)
%     %     a=1
