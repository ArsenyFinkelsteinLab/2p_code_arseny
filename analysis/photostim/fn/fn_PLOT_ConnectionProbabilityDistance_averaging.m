function OUT=fn_PLOT_ConnectionProbabilityDistance_averaging(D,D_all,distance_axial_bins, flag_response,  maxv, minv)
OUT=[];
...
for j=1:1:numel(idx_sessions)
    ...
    switch flag_response
        case 0 %all
            current_map = D(ii).counts_2d./D_all(ii).counts_2d;
        ...
        case 1 %excitation
            current_map = D(ii).counts_2d_excitation./D_all(ii).counts_2d;
            current_lateral = sum(D(ii).counts_2d_excitation,1)./sum(D_all(ii).counts_2d,1);
            current_axial =sum(D(ii).counts_2d_excitation,2)./sum(D_all(ii).counts_2d,2);
            current_axial_in_column =sum(D(ii).counts_2d_excitation,2)./sum(D_all(ii).counts_2d,2);
            current_axial_out_column = sum(D(ii).counts_2d_excitation,2)./sum(D_all(ii).counts_2d,2);
        case 2 %inhibition
...
if nargin<5 % if the scaling values were not provided as inputs
    maxv=prctile(map(:),99);
