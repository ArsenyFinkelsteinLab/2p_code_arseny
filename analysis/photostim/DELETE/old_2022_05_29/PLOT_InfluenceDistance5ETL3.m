function PLOT_InfluenceDistance5ETL3()
...
rel_data = (STIMANAL.InfluenceDistance55ETL4 & 'flag_divide_by_std=0' & 'flag_withold_trials=0' & 'flag_normalize_by_total=1') ...
    &  (STIMANAL.SessionEpochsIncluded& IMG.Volumetric & 'stimpower>=100' & 'flag_include=1' & 'session_epoch_number<=3' ...
    & (STIMANAL.NeuronOrControl5ETL3 & 'neurons_or_control=1' & 'num_targets>=50') ...
    & (STIMANAL.NeuronOrControl5ETL3 & 'neurons_or_control=0' & 'num_targets>=0'));
...
imagesc(OUT1.distance_lateral_bins_centers(2:end), distance_axial_bins_plot,  OUT1.map(:,2:end))
axis tight
axis equal
cmp = bluewhitered(512); %
colormap(ax1, cmp)
caxis([OUT1.minv max(max(OUT1.map(:,2:end)))]);
