function photostim_coupled_distance_binning()
...
rel=STIM.ROIResponse & 'response_p_value<=0.001';
...
distance_bins_microns = [0,30,60,90,120,200,300,600];
...
%     rel_group_targets=rel & (IMG.PhotostimGroup & (IMG.PhotostimGroupROI & (STIM.ROIResponseDirect &'response_p_value<0.01'))& k1);
    ...
    for i_d=1:1:numel(distance_bins_microns)-1
        ...
        y_m = trace_mean_by_distance_excit(i_d,:);
        %        y_smooth=  y_m;
        ...
        title(sprintf('[%d %d] um\n %d cells',distance_bins_microns(i_d),distance_bins_microns(i_d+1),num_roi_bin_excit(i_d)),'FontSize',10);
