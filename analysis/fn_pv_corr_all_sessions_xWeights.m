function fn_pv_corr_all_sessions_xWeights(k, first_date, dir_save_figure, rel ,mode_name)
...
for i_condition = 1:numel(trials)
    ...
    for iTr=1:1:numel(current_trials)
        ...
        key_mode=fetch(EXP2.SessionTrial & key);
        key_mode.mode_type_name = mode_name;
        w=fetchn(ANLI.Mode & rel & key_mode,'mode_unit_weight', 'ORDER BY roi_number');
        PV.epoch{1}(:,iTr)= mean(psth_trial(:,t_delay_idx),2).*abs(w);
        PV.epoch{2}(:,iTr)= mean(psth_trial(:,t_response_idx),2).*abs(w);
...
filename=['pv_' first_date '_' mode_name];
